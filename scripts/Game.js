    ...
    findPath(startx,starty,finishx,finishy){
        ...
        for(let loopcount = 0; loopcount<9*9;loopcount++){
            ...
            neighbors.forEach((neighbor) => {
                let currentDis = distance[current.x][current.y] +1;
                if(marked[neighbor.x][neighbor.y]==false &&  currentDis < distance[neighbor.x][neighbor.y]){
                    prev[neighbor.x][neighbor.y] = current;
                    distance[neighbor.x][neighbor.y] = currentDis;
                 }
            });
